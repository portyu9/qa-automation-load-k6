...
const K6_DURATION = /^(?:\d+(?:\.\d+)?(?:ms|s|m|h))+$/;
const DURATION_PART = /(\d+(?:\.\d+)?)(ms|s|m|h)/g;
const DURATION_MULTIPLIER_MS = Object.freeze({ ms: 1, s: 1_000, m: 60_000, h: 3_600_000 });
...
function positiveDuration(name, fallback) {
  const raw = __ENV[name];
  const value = String(raw === undefined || raw === '' ? fallback : raw).trim();
  if (!K6_DURATION.test(value)) {
    throw new Error(`${name} must be a positive k6 duration using ms, s, m, or h units`);
  }

  let totalMs = 0;
  for (const match of value.matchAll(DURATION_PART)) {
    totalMs += Number(match[1]) * DURATION_MULTIPLIER_MS[match[2]];
  }
...
export const config = Object.freeze({
  ...
  soakDuration: positiveDuration('K6_SOAK_DURATION', '10m'),
  soakRate: positiveNumber('K6_SOAK_RATE', 5),
});
